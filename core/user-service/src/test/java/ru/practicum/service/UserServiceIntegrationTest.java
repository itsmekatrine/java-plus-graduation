...
class UserServiceIntegrationTest extends IntegrationTestBase {
    ...
    void create_whenEmailIsBusy() {
        ...
        Assertions.assertThatThrownBy(() -> service.create(NewUserRequest.builder()
                .name(name)
                .email(email)
    ...
    void delete_whenUserNotExists() {
        long userId = 999L;
        Assertions.assertThatThrownBy(() -> service.delete(userId)).isInstanceOf(NotFoundException.class);
    }
    ...
    void getUsers_withIds() {
        ...
        Assertions.assertThat(users).last()
